...
function isString(value) {
  ...
  if (value === '') {
    ...
    isStatus = value ? value.constructor === String : false;
  }
  return isStatus;
}
...
function removeFirstOccurrences(str, value) {
  ...
  if (index === -1) {
    ...
    const part2 = str.substring(index + value.length, str.length);
    newStr = part1.concat(part2);
  }
  return newStr;
}
...
function formatTime(minutes, seconds) {
  ...
  return newMin.concat(':'.concat(newSec));
}
...
function countVowels(str) {
  ...
  let count = 0;
  for (let indexStr = 0; indexStr < str.length; indexStr += 1) {
    for (let indexVowels = 0; indexVowels < vowels.length; indexVowels += 1) {
      count = str[indexStr] === vowels[indexVowels] ? count + 1 : count;
    }
  }
  return count;
}
...
function isPalindrome(str) {
  let newStr = str.toLocaleLowerCase().replaceAll(/[\s.,!?%]/g, '');
  newStr = newStr.split('').reverse().join('');
  return str.toLocaleLowerCase().replaceAll(/[\s.,!?%]/g, '') === newStr;
}
...
function extractNameFromTemplate(value) {
  const array = value.replaceAll(/[,!%]/g, '').split(' ');
...
function encodeToRot13(str) {
  ...
  for (let index = 0; index < arrayStr.length; index += 1) {
    if (lowers.indexOf(arrayStr[index]) !== -1) {
      arrayStr[index] = lowers[lowers.indexOf(arrayStr[index]) + 13];
    } else if (uppers.indexOf(arrayStr[index]) !== -1) {
      arrayStr[index] = uppers[uppers.indexOf(arrayStr[index]) + 13];
    }
  }
...
function getCardId(value) {
  let id;
  const newValue = value.replace(value[value.length - 1], '');
  switch (newValue) {
    case 'A':
      id = 0;
      break;
